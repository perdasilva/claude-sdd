...
Interactively set up spec-driven development in the current project. It detects whether the project is greenfield or brownfield, walks you through defining mission, tech stack, roadmap, and conventions as spec documents, then generates five project-specific workflow commands (`/sdd-plan-next-phase`, `/sdd-implement`, `/sdd-review`, `/sdd-ship`, `/sdd-ideate`) customized to your build tools and PR conventions.

Accepts an optional initial description to front-load project context (e.g., `/bootstrap A REST API in Python using FastAPI...`). When provided, Claude pre-fills what it can and focuses questions on gaps.
